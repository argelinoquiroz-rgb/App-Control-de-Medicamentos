...
def load_records():
    if os.path.exists(DATA_FILE):
        ...
        # Convertir None/NaN a ""
        df = df.fillna("")
        # Asegurar columna consecutivo
...
def append_record(record: dict):
    df = load_records()
    record["consecutivo"] = str(df["consecutivo"].max() + 1 if not df.empty else 1)
    # Reemplazar None por ""
    ...
    df.to_csv(DATA_FILE, index=False)
    return record["consecutivo"]

def save_support_file(uploaded_file, nombre_pdf):
    path = os.path.join(SOPORTES_DIR, nombre_pdf)
...
def page_registrar():
    ...
    if st.button("💾 Guardar registro"):
        if not (plu and nombre and soporte):
            ...
            # Generar consecutivo
            df = load_records()
            consecutivo = str(df["consecutivo"].max() + 1 if not df.empty else 1)
            ext = os.path.splitext(soporte.name)[1]
            nombre_pdf = f"{consecutivo}_{nombre.replace(' ','_')}{ext}"
            ruta_soporte = save_support_file(soporte, nombre_pdf)
            registro = {
...
def page_registros():
    ...
    search_term = st.text_input("🔍 Buscar registros por cualquier campo").strip().lower()
    if search_term:
    ...
    expected_cols = ["consecutivo","fecha_hora","usuario","estado","plu","codigo_generico",
                     "nombre_comercial","laboratorio","presentacion","observaciones","soporte"]
    for col in expected_cols:
        if col not in df_filtered.columns:
            df_filtered[col] = ""

    # Mostrar tabla con botones de descarga
    st.write("### 📋 Registros")
    for idx, row in df_filtered.iterrows():
        col1, col2, col3, col4, col5, col6, col7, col8, col9, col10 = st.columns(10)
        col1.write(row.get("consecutivo",""))
        col2.write(row.get("fecha_hora",""))
        col3.write(row.get("usuario",""))
        col4.write(row.get("estado",""))
        col5.write(row.get("plu",""))
        col6.write(row.get("codigo_generico",""))
        col7.write(row.get("nombre_comercial",""))
        col8.write(row.get("laboratorio",""))
        col9.write(row.get("presentacion",""))
        col10.write(row.get("observaciones",""))
        # Botón descarga
        if os.path.exists(row.get("soporte","")):
            with open(row["soporte"],"rb") as f:
                data_bytes = f.read()
            ext = os.path.splitext(row["soporte"])[1]
            nombre_pdf = f"{row['consecutivo']}_{row['nombre_comercial'].replace(' ','_')}{ext}"
            st.download_button(label="📥 Descargar", data=data_bytes, file_name=nombre_pdf, mime=guess_mime(row["soporte"]))
