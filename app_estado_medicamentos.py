...
def save_support_file(uploaded_file, consecutivo, nombre):
    # Generar nombre del archivo: consecutivo_nombre-medicamento.pdf
...
def page_registros():
    ...
    df_display["Descargar"] = "📥 Descargar"

    # Configurar AgGrid
    gb = GridOptionsBuilder.from_dataframe(df_display)
    gb.configure_selection("single")
    gb.configure_column("soporte", hide=True)
    gb.configure_column("Descargar", editable=False)
    ...
    # Descargar soporte si se selecciona una fila
    selected = grid_response["selected_rows"]
    if selected:
        selected_row = selected[0]
        soporte_path = selected_row["soporte"]
        if os.path.exists(soporte_path):
            with open(soporte_path,"rb") as f:
                data_bytes = f.read()
            st.download_button("📥 Descargar PDF seleccionado",
                               data=data_bytes,
