...
def save_support_file(uploaded_file, consecutivo, plu, nombre):
    ...
    safe_name = f"{nombre.replace(' ','_')}_{consecutivo}{ext}"
...
def page_registrar():
    ...
    explicaciones_estado = {
        "Agotado":"🟡 No disponible temporalmente en inventario interno.",
        "Desabastecido":"🔴 No disponible ni en inventario interno ni mercado nacional.",
        "Descontinuado":"⚫ Retirado del mercado por el fabricante o autoridad sanitaria."
    }
...
def page_registros():
    ...
    if df.empty:
        ...
        return
    search_term = st.text_input("🔍 Buscar registro (por cualquier campo)").strip().lower()
    if search_term:
    ...
    for idx,row in df_filtered.iterrows():
        soporte_nombre = row.get("soporte","")
        ruta_soporte = os.path.join(SOPORTES_DIR, soporte_nombre)
        if os.path.exists(ruta_soporte):
            with open(ruta_soporte,"rb") as f:
                data_bytes = f.read()
            st.download_button(
                label=f"📥 Descargar soporte - {row.get('nombre_comercial','')}",
                data=data_bytes,
                file_name=soporte_nombre,
                mime=guess_mime(ruta_soporte)
            )
        else:
            st.warning(f"Soporte no encontrado para registro {row.get('consecutivo')}")
