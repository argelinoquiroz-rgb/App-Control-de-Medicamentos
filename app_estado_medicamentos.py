...
def append_record(record: dict):
    df = load_records()
    record["consecutivo"] = str(len(df)+1)
...
def save_support_file(uploaded_file, nombre_pdf):
    safe_name = "".join(c if c.isalnum() or c in "_-." else "_" for c in nombre_pdf)
    ext = os.path.splitext(uploaded_file.name)[1].lower()
    full_name = f"{safe_name}{ext}"
    path = os.path.join(SOPORTES_DIR, full_name)
    ...
    if st.button("💾 Guardar registro"):
        if not (plu and nombre and soporte):
            ...
            registro = {
                ...
                "estado": estado or "",
                "plu": plu or "",
                "codigo_generico": codigo_gen or "",
                "nombre_comercial": nombre or "",
    ...
    for idx,row in df_filtered.iterrows():
        ...
        if os.path.exists(soporte_path):
            with open(soporte_path,"rb") as f:
                data_bytes = f.read()
            ext = os.path.splitext(soporte_path)[1]
