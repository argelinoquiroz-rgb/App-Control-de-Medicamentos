...
def append_record(record: dict):
    df = load_records()
    record["consecutivo"] = str(len(df)+1)  # asignar consecutivo automático
...
def login_page():
    ...
    st.markdown("<p style='text-align:center;'>Inicie sesión para acceder al sistema.</p>", unsafe_allow_html=True)
...
def page_registrar():
    ...
    if st.button("💾 Guardar registro"):
        if not (plu and nombre and soporte):
            ...
            nombre_pdf = f"{consecutivo}_{nombre.replace(' ','_')}"
...
def page_registros():
    ...
    # Tabla con todos los registros excepto la ruta del soporte
    st.dataframe(df[["consecutivo","fecha_hora","usuario","estado","plu","codigo_generico",
    ...
    for idx,row in df.iterrows():
        ...
        if os.path.exists(soporte_path):
            with open(soporte_path,"rb") as f:
                data_bytes = f.read()
            st.download_button(
                label=f"📥 Descargar {row['consecutivo']}_{row['nombre_comercial']}",
                data=data_bytes,
